...
DOCKER_CONTAINER_NUMBER=$(docker ps | grep "${CONTAINER_KEYWORD}" | awk 'END{print FNR}')

if [ ${DOCKER_CONTAINER_NUMBER} -gt "${MAX_CONTAINER_NUMBER}" ]; then
  DOCKER_CONTAINER_LIST=$(docker ps | grep "${CONTAINER_KEYWORD}" | awk  '{print $10, $14}' | tail -"${DELETE_CONTAINER_NUMBER}" | awk '{printf $2 " " }')
  echo "Container Count: ${DOCKER_CONTAINER_NUMBER} > ${MAX_CONTAINER_NUMBER}, will delete oldest ${DELETE_CONTAINER_NUMBER}"
